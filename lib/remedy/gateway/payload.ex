defmodule Remedy.Gateway.Payload do
  ...
  defmacro __using__(_) do
    ...
    quote do
      ...
      @doc false
      def build_payload(socket, opts, default \\ []) do
        Payload.send(event_from_mod(), payload(socket, opts || default))
      end
      ...
      def do_digest(socket, payload) do
        Payload.digest(event_from_mod(), digest(socket, payload), socket)
      end
  ...
  def send(command, {:noop, socket}) do
    socket
  end
  ...
  def send(socket, event, opts \\ []) when is_op_event(event) do
    Module.concat([event]).send_payload(socket, opts)
  end
  ...
  def digest(socket, frame) do
    ...
    Module.concat([event]).digest(socket, payload)
  end
